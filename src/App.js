...
class App extends React.Component {
    ...
    if (query) {
      api
        ...
        .then((res) => {
          ...
          this.setState(
            { movies: results, initialMovies: results, loading: false },
            this.sort
          );
        });
    } else {
  ...
  sort = () => {
    ...
    if (sort === 'clear') {
      ...
      this.setState({
        movies: MOVIES.sort((a, b) => (a[sort] < b[sort] ? 1 : -1)),
      });
    }
  };
    ...
    return (
      <div className="app">
        <header className="header">
          <Logo width="80" />
          <h1 className="title">Film Bibliothek</h1>
        </header>
        <form className="search-container" onSubmit={this.handleSearch}>
          ...
          {query && (
            <button
              className="form-action fas fa-times"
        ...
        <div className="grid">
          {movies.map((movie) => (
            <div
              ...
              key={movie.id}
              onClick={() => this.setState({ movieID: movie.id })}
            >
              <img
